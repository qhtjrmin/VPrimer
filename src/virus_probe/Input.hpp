...
bool readInputParameter(int argc, char*argv[], inputParameter* input) {
	...
	while (++argnr < argc)
	{
		if (!strcmp(argv[argnr], "-i")) {
			...
			tmpFname = dir + "/C1.txt";
//			tmpFname = "/mnt/min/data_virus/virus_total/probe/C1.txt";
//			tmpFname = "/mnt/min/updated_result/SARS_CoV2/probe/C1.txt";
			strcpy(input->c1Path, tmpFname.c_str());

			tmpFname = dir + "/myC1'.txt";
			strcpy(input->myc1SidsetPath, tmpFname.c_str());
		...
		else if(!strcmp(argv[argnr], "-h")){
			input->c1SidsetPath = argv[++argnr];
		}

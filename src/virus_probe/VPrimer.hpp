...
//	sysCall("cat " + string(myInput->dirPath) + "/tmpC1.txt_* >> " +
//			totalFile);
...
/*		else{//preparing arraysC3 for RPs
			...
			myArraysC3[tmpRank]->SIDR = new unsigned int[10 * myPRsize];
			...
			for (unsigned long i = tmpRank; i < seedHrsize; i += numOfGPUs) {
				...
				if (index > myPRsize || left > myseedHrsize || sidIdx > 10 * myPRsize) {
					cout << "ERRR!! " << index << "/" << myPRsize << " " << sidIdx
							<< "/" << 10 * myPRsize << " " << left << "/" << myseedHrsize
							<< endl;
				}
...
void stage5Sort(){
	...
	command = "sort -k2,2n -k1,1 -S 50% --parallel "
			+ to_string(myInput->numOfThreads) + " -T " + string(myInput->dirPath)
			+ " " + totalFile + " -o " + sortedFile;
	sysCall(command);

	command = "cp " + sortedFile + " " + stage5_input;
	sysCall(command);

...
//	sysCall("rm " + string(myInput->dirPath) + "/total.txt");
//	sysCall("rm " + string(myInput->dirPath) + "/sorted.txt");
	...
	if (input->isWriteOutput) {
		cout << "## Stage 3 (write) : 5' cross hybridization writing" << endl;
	...
	for (int k = 1; k <= 2; k++) {
		...
		if(input->isWriteOutput || k == 2){
			cout << "## Stage 4 (write) : general cross hybridization writing" << endl;
	...
	runPthreadParam(writeOutput2, input->numOfThreads, 1, &elapsedTime);

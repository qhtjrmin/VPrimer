...
void stage1Sort(){
	...
	//sort by primer
	command = "sort -k1,1 -k2,2n -S 80% --parallel "
			+ to_string(myInput->numOfThreads) + " -T " + "/data/ssd2/min/"  + " "
...
void* stage4Building(void* param) {
	...
	unsigned int primerHwrokload = primerH.size() / numOfThreads + 1;
	...
	for (auto it_primerH = next(primerH.begin(), primerHwrokload * myRank);
			it_primerH != primerH.end(); it_primerH++) {
		if (valid_cnt < primerHwrokload) {
			if (it_primerH->second == true) {
				...
				for (int i = 0; i < primerLen - seedLen + 1; i++) {
					...
					if ((valid_cnt >= cnt * 100000) && (myRank == 0)) {
						cout << valid_cnt << " / " << primerHwrokload << " : "
								<< primer << " " << newKey << endl;
						cnt++;
					}
					#endif
...
void* stage4Prepare(void* param) { //prepare arraysC3 and copy it to GPU memory
	...
	if(myRank < 2 * numOfGPUs){
		...
		if(myRank % 2 == 0){ //preparing arraysC3 for FPs
			...
			myArraysC3[tmpRank]->SIDF = new unsigned int[300 * myPFsize];
			...
			for (unsigned long i = tmpRank; i < seedHfsize; i += numOfGPUs) {
				...
				if (index > myPFsize || left > myseedHfsize || sidIdx > 300 * myPFsize) {
					cout << "ERRF!! " << index << "/" << myPFsize << " " << sidIdx
							<< "/" << 300 * myPFsize << " " << left << "/" << seedHfsize
							<< endl;
				}
			...
			myArraysC3[tmpRank]->SIDR = new unsigned int[300 * myPRsize];
			...
			for (unsigned long i = tmpRank; i < seedHrsize; i += numOfGPUs) {
				...
				if (index > myPRsize || left > myseedHrsize || sidIdx > 300 * myPRsize) {
					cout << "ERRR!! " << index << "/" << myPRsize << " " << sidIdx
							<< "/" << 300 * myPRsize << " " << left << "/" << myseedHrsize
							<< endl;
				}
...
//	totalFile = string(myInput->dirPath) + "/total.txt";
//	sortedFile = string(myInput->dirPath) + "/sorted.txt";

//	fin.open(totalFile);
//	if(fin) sysCall("rm " + totalFile);
//	sysCall("cat " + string(myInput->c4Path2) + "_* >> " + totalFile);
//	fin.close();
}
...
//	totalFile = string(myInput->dirPath) + "/total.txt";
	sortedFile = string(myInput->dirPath) + "/sorted.txt";
//	stage5_input = "/data/ssd2/min/HPrimer/MRPrimerV2_input/test2/sorted.txt";
	command = "sort -k2,2n -k1,1 -S 60% --parallel "
...
void probeBuild(){
	...
	ifstream fin;
	fileReadOpen(&fin, myInput->probePath, -1);
				...
				*ptr2 = '\0';
				...
				if (myTurn) {
					...
					*ptr3 = '\0';
					...
					if(probeH.find(ptr2) != probeH.end()){
					tmp_sid = stoi(ptr2);
					//tmp_sid2 = ptr2;
					tmp_pos = stoi(++ptr3);
...
void *stage5(void* rank) {
	...
	if (myRank == 0) {
		...
		cout << "each-thread mode : " << et2 - st2 << " sec (gpu: "
				<< (et2 - st2) - (et1 - st1) << " / wt: " << (et1 - st1) << ")"
				<< endl;
	}
	...
	if (myRank == 0) {
		...
		cout << "block-threads mode : " << et2 - st2 << " sec (wt: "
				<< et1 - st1 << ")" << endl;
	}
	...
	if (myRank < numOfGPUs) {
		...
		if (myRank == 0) {
			cout << "passed : " << total_passed << endl;
			cout << "all-threads mode : " << et2 - st2 << " sec (wt: "
					<< et1 - st1 << ")" << endl;
		}
	}
...
void finalSort(){
	...
	command = "cat " + string(myInput->outputPath) + "_* >> " + totalFile;
	sysCall(command);

	sysCall("rm " + string(myInput->outputPath) + "_*");

	command = "sort -k1,1 -k3,3n -S 70% --parallel " + to_string(myInput->numOfThreads)
			+ " -T " + string(myInput->dirPath) + " " + totalFile + " -o "
			+ string(myInput->outputPath);
	sysCall(command);

	//sysCall("rm " + totalFile);
}
